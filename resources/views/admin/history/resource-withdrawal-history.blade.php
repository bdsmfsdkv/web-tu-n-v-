                            ...
                            <tbody>
                                @forelse($withdrawals as $withdrawal)
                                    <tr>
                                        ...
                                        <td><span class="text-muted">{{ $withdrawal->server }}</span></td>
                                        <td>
                                            {!! display_status_admin($withdrawal->status) !!}
                                        </td>
                                        <td><span class="text-muted small">{{ $withdrawal->created_at->format('d/m/Y H:i') }}</span></td>
                                        <td class="text-end">
                                            @if ($withdrawal->status === 'processing')
                                                <div class="btn-group">
                                                    <button class="btn btn-sm btn-light-success text-success" data-bs-toggle="modal"
                                                        data-bs-target="#successModal{{ $withdrawal->id }}" title="Duyệt">
                                                        <i class="ti ti-check"></i>
                                                    </button>
                                                    <button class="btn btn-sm btn-light-danger text-danger" data-bs-toggle="modal"
                                                        data-bs-target="#errorModal{{ $withdrawal->id }}" title="Từ chối">
                                                        <i class="ti ti-x"></i>
                                                    </button>
                                                </div>
                                                            ...
                                                            <form
                                                                ...
                                                                <div class="modal-body">
                                                                    <div class="alert alert-warning mb-3">Bạn có chắc chắn muốn duyệt yêu cầu rút
                                                                        <strong>{{ number_format($withdrawal->amount) }}</strong>
                                                                        {{ $withdrawal->type === 'gold' ? 'vàng' : 'ngọc' }}
                                                                        này?</div>
