        ...
        <div class="col-md-12">
            ...
            <div class="page-btn">
                <a href="javascript:void(0);" class="btn btn-success" data-bs-toggle="modal" data-bs-target="#addFlashSaleModal">
                    <img src="{{ asset('assets/admin/img/icons/plus.svg') }}" alt="img" class="me-1"> Tạo Flash Sale mới
                </a>
            </div>
        </div>
                ...
                <div class="table-responsive table-border-style">
                    <table class="table table-hover table-borderless align-middle mb-0 text-nowrap w-100">
                        ...
                        <tbody>
                                ...
                                <tr>
                                    <td><strong>{{ $fs->campaign_name }}</strong></td>
                                    <td>{{ $fs->items->count() }} sản phẩm</td>
                                    <td>{{ \Carbon\Carbon::parse($fs->start_time)->format('d/m/Y H:i') }}</td>
                                    <td>{{ \Carbon\Carbon::parse($fs->end_time)->format('d/m/Y H:i') }}</td>
                                    <td>
                                        @if($fs->status == 1 && \Carbon\Carbon::now()->lt(\Carbon\Carbon::parse($fs->end_time)))
                                    ...
                                    <td>
                                        <form action="{{ route('admin.flash-sales.destroy', $fs->id) }}" method="POST" style="display:inline;" onsubmit="return confirm('Bạn có chắc chắn muốn xóa chiến dịch Flash Sale này?');">
                                            @csrf
                                            @method('DELETE')
                                    ...
                                    <div class="col-md-4 mb-2 mb-md-0">
                                        <select name="products[0][id]" class="form-select" required>
                                            ...
                                            <optgroup label="Tài Khoản Game">
                                                @foreach($allGameCategories as $cat)
                                                    <option value="game_{{ $cat->id }}">{{ $cat->name }}</option>
                                                @endforeach
                                            </optgroup>
                                            <optgroup label="Tài Khoản Random">
                                                @foreach($allRandomCategories as $cat)
                                                    <option value="random_{{ $cat->id }}">{{ $cat->name }}</option>
                                                @endforeach
                                            </optgroup>
                                        </select>
                                    </div>
            ...
            <div class="row align-items-center product-item mb-3 g-2">
                <div class="col-md-4 mb-2 mb-md-0">
                    <select name="products[${productIndex}][id]" class="form-select" required>
                        ...
                        <optgroup label="Tài Khoản Game">
                            @foreach($allGameCategories as $cat)
                                <option value="game_{{ $cat->id }}">{{ str_replace("'", "\'", $cat->name) }}</option>
                            @endforeach
                        </optgroup>
                        <optgroup label="Tài Khoản Random">
                            @foreach($allRandomCategories as $cat)
                                <option value="random_{{ $cat->id }}">{{ str_replace("'", "\'", $cat->name) }}</option>
                            @endforeach
                        </optgroup>
                    </select>
                </div>
