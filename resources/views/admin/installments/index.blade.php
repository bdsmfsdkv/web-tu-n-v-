...
@section('title', 'Quản lý mua trả góp')
@section('content')
    <div >
        <div >
                            ...
                            <tbody>
                                @forelse($installments as $installment)
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <strong>{{ $installment->user->username ?? 'Unknown' }}</strong><br>
                                            <small>{{ $installment->user->email ?? '' }}</small>
                                        </td>
                                        <td>
                                            <a href="{{ route('account.show', $installment->game_account_id) }}" target="_blank">Nick #{{ $installment->game_account_id }}</a><br>
                                            <small>{{ $installment->gameAccount->category->name ?? '' }}</small>
                                        </td>
                                        <td class="text-bolds">{{ number_format($installment->total_price) }}đ</td>
                                        <td style="color: #10b981; font-weight: bold;">{{ number_format($installment->paid_amount) }}đ</td>
                                        <td style="color: #ef4444; font-weight: bold;">{{ number_format($remaining) }}đ</td>
                                        <td>
                                        ...
                                        <td>
                                            @if($installment->status === 'active')
                                                <form action="{{ route('admin.installments.cancel', $installment->id) }}" method="POST" style="display:inline-block;" onsubmit="return confirm('Bạn có chắc chắn muốn hủy hợp đồng này và mở bán lại nick không?');">
                                                    @csrf
