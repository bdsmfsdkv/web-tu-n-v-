...
@section('title', 'Kho Thưởng - Quản lý vật phẩm thưởng')
@section('content')
    <div>
        <div class="page-header">
            ...
            <div class="page-btn">
                <a href="{{ route('admin.reward-items.create') }}" class="btn btn-warning text-white fw-bold">
        ...
        <div class="card overflow-hidden shadow-sm border border-dashed">
            <div class="card-body px-0 py-0">
                <form class="p-3 bg-auto-subtle border-bottom" action="{{ route('admin.reward-items.index') }}" method="GET">
                    <div class="row align-items-center g-2">
                        ...
                        <div class="col-md-3 d-flex gap-2 ms-auto">
                            ...
                            <a href="{{ route('admin.reward-items.index') }}" class="btn btn-sm btn-light-danger w-100">
                    ...
                    <table class="table table-hover table-borderless align-middle mb-0 text-nowrap w-100">
                        ...
                        <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <span class="badge bg-primary px-3 py-2 rounded">{{ $item->game_name }}</span>
                                    </td>
                                    <td><span class="fw-bold text-auto">{{ $item->name }}</span></td>
                                    <td>
                                        <span class="badge bg-info px-3 py-2 rounded">{{ $item->unit }}</span>
                                    </td>
                                    <td><span class="text-muted">{{ $item->code }}</span></td>
                                    <td>{{ number_format($item->min_withdraw) }}</td>
                                    <td>{{ number_format($item->max_withdraw) }}</td>
                                    <td>{{ $item->priority }}</td>
                                    <td class="text-center">
                                        @if ($item->active)
                                    ...
                                    <td class="text-center">
                                        <a class="me-2 text-primary" href="{{ route('admin.reward-items.edit', $item->id) }}" title="Sửa">
                                            <i class="ti ti-edit fs-5"></i>
                                        </a>
                                        <a class="text-danger delete-item" href="javascript:void(0);" data-id="{{ $item->id }}" title="Xóa">
    ...
    <x-modal-confirm-delete message="Bạn có chắc chắn muốn xóa vật phẩm này không?" />
@endsection
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            $('#confirmDelete').on('click', function() {
                $.ajax({
                    url: "{{ route('admin.reward-items.destroy', '') }}/" + itemId,
                    type: 'DELETE',
                    data: {
                        _token: '{{ csrf_token() }}'
                    },
