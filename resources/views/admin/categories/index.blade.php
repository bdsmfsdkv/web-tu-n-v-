@extends('layouts.admin.app')
@section('title', 'Quản lý danh mục game')
@section('content')
    <div>
        <div>
            <div class="page-header">
                <div class="page-block mb-3">
                    <div class="row align-items-center">
                        <div class="col-md-12">
                            <div class="page-header-title">
                                <h2 class="mb-0">DANH SÁCH DANH MỤC GAME</h2>
                                <p class="text-muted">Quản lý danh mục game của bạn</p>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="page-btn">
                    <a href="{{ route('admin.categories.create') }}" class="btn btn-success">
                        <i class="ti ti-plus me-1"></i>Thêm danh mục mới
                    </a>
                </div>
            </div>

            @if (session('success'))
                <x-alert-admin type="success" :message="session('success')" />
            @endif

            @if (session('error'))
                <x-alert-admin type="danger" :message="session('error')" />
            @endif

            <div class="card overflow-hidden shadow-sm border border-dashed">
                <div class="card-body px-0 py-0">
                    <form class="p-3 bg-auto-subtle border-bottom filter-form" method="GET">
                        <div class="row align-items-center g-2">
                            <div class="col-md-2">
                                <select name="per_page" class="form-select form-select-sm" onchange="this.form.submit()">
                                    <option value="">--Hiển thị--</option>
                                    <option value="10" {{ request('per_page') == 10 ? 'selected' : '' }}>10</option>
                                    <option value="25" {{ request('per_page') == 25 ? 'selected' : '' }}>25</option>
                                    <option value="50" {{ request('per_page') == 50 ? 'selected' : '' }}>50</option>
                                    <option value="100" {{ request('per_page') == 100 ? 'selected' : '' }}>100</option>
                                </select>
                            </div>
                            <div class="col-md-3">
                                <input type="text" name="search" class="form-control form-control-sm" placeholder="Tìm kiếm..." value="{{ request('search') }}">
                            </div>
                            <div class="col-md-3 d-flex gap-2 ms-auto">
                                <button type="submit" class="btn btn-sm btn-primary w-100">
                                    <i class="ti ti-search me-1"></i> Tìm kiếm
                                </button>
                                <a href="?" class="btn btn-sm btn-light-danger w-100">
                                    <i class="ti ti-trash me-1"></i> Bỏ lọc
                                </a>
                            </div>
                        </div>
                    </form>

                    <div class="table-responsive table-border-style">
                        <table class="table table-hover table-borderless align-middle mb-0 text-nowrap w-100">
                            <thead class="bg-light-subtle text-muted">
                                <tr>
                                    <th class="text-uppercase small">
                                        <div class="form-check"><input class="form-check-input" type="checkbox" id="select-all"></div>
                                    </th>
                                    <th class="text-uppercase small">Order</th>
                                    <th class="text-uppercase small">ID</th>
                                    <th class="text-uppercase small">Tên danh mục</th>
                                    <th class="text-uppercase small">Ảnh đại diện</th>
                                    <th class="text-uppercase small">Trạng thái</th>
                                    <th class="text-uppercase small">Ngày tạo</th>
                                    <th class="text-uppercase small">Thao tác</th>
                                </tr>
                            </thead>
                            <tbody>
                                @forelse($categories as $key => $category)
                                    <tr>
                                        <td>
                                            <div class="form-check"><input class="form-check-input" type="checkbox"></div>
                                        </td>
                                        <td>
                                            <span class="badge bg-light-primary fw-semibold">
                                                {{ $categories->firstItem() + $key }}
                                            </span>
                                        </td>
                                        <td>{{ $category->id }}</td>
                                        <td class="text-bolds">{{ $category->name }}</td>
                                        <td>
                                            <img src="{{ asset($category->thumbnail) }}" alt="{{ $category->name }}"
                                                class="img-thumbnail" style="max-width: 200px;">
                                        </td>
                                        <td>
                                            <span class="badge {{ $category->active ? 'bg-lightgreen' : 'bg-lightred' }}">
                                                {{ $category->active ? 'Hoạt động' : 'Đã ẩn' }}
                                            </span>
                                        </td>
                                        <td>{{ $category->created_at->format('d/m/Y') }}</td>
                                        <td>
                                            <a class="me-3" href="{{ route('admin.categories.edit', $category->id) }}">
                                                <i class="ti ti-edit fs-5 text-primary"></i>
                                            </a>
                                            <a class="me-3 confirm-delete" href="javascript:void(0);" data-bs-toggle="modal"
                                                data-bs-target="#deleteModal" data-id="{{ $category->id }}">
                                                <i class="ti ti-trash fs-5 text-danger"></i>
                                            </a>
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="100%">
                                            <div class="text-center py-5">
                                                <svg style="width: 184px; height: 152px;" viewBox="0 0 184 152" xmlns="http://www.w3.org/2000/svg">
                                                    <g fill="none" fill-rule="evenodd">
                                                        <g transform="translate(24 31.67)">
                                                            <ellipse fill-opacity=".8" fill="#F5F5F7" cx="67.797" cy="106.89" rx="67.797" ry="12.668"></ellipse>
                                                            <path d="M122.034 69.674L98.109 40.229c-1.148-1.386-2.826-2.225-4.593-2.225h-51.44c-1.766 0-3.444.839-4.592 2.225L13.56 69.674v15.383h108.475V69.674z" fill="#AEB8C2"></path>
                                                            <path d="M33.83 0h67.933a4 4 0 0 1 4 4v93.344a4 4 0 0 1-4 4H33.83a4 4 0 0 1-4-4V4a4 4 0 0 1 4-4z" fill="#F5F5F7"></path>
                                                            <path d="M42.678 9.953h50.237a2 2 0 0 1 2 2V36.91a2 2 0 0 1-2 2H42.678V11.953a2 2 0 0 1 2-2z" fill="#DCE0E6"></path>
                                                            <path d="M42.94 49.767h49.713a2.262 2.262 0 1 1 0 4.524H42.94a2.262 2.262 0 1 1 0-4.524zM42.94 61.53h49.713a2.262 2.262 0 1 1 0 4.525H42.94a2.262 2.262 0 1 1 0-4.525z" fill="#DCE0E6"></path>
                                                        </g>
                                                    </g>
                                                </svg>
                                                <p class="mt-3 text-muted">Không tìm thấy dữ liệu</p>
                                            </div>
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>

                    @php
                        $paginator = null;
                        foreach(get_defined_vars() as $var) {
                            if (is_object($var) && method_exists($var, 'hasPages')) {
                                $paginator = $var;
                                break;
                            }
                        }
                    @endphp
                    @if($paginator && $paginator->hasPages())
                        <div class="d-flex justify-content-end p-3 border-top">
                            {{ $paginator->withQueryString()->links('pagination::bootstrap-5') }}
                        </div>
                    @endif
                </div>
            </div>
        </div>
    </div>

    <x-modal-confirm-delete
        message="Nếu bạn chỉ muốn ẩn danh mục khỏi website, hãy tắt trạng thái Hoạt động. Nếu xác nhận xóa, danh mục và toàn bộ tài khoản game thuộc danh mục này sẽ được xóa theo." />
@endsection

@push('scripts')
    <script>
        $(document).ready(function() {
            let userId;

            $('.confirm-delete').on('click', function() {
                userId = $(this).data('id');
            });

            $('#confirmDelete').on('click', function() {
                $.ajax({
                    url: '/admin/categories/delete/' + userId,
                    type: 'DELETE',
                    data: {
                        _token: '{{ csrf_token() }}'
                    },
                    success: function(response) {
                        $('#deleteModal').modal('hide');
                        if (response.success) {
                            Swal.fire({
                                icon: 'success',
                                title: 'Thành công!',
                                text: response.message || 'Đã xóa danh mục game thành công',
                                showConfirmButton: false,
                                timer: 1800
                            }).then(() => {
                                window.location.reload();
                            });
                        } else {
                            Swal.fire({
                                icon: 'error',
                                title: 'Lỗi!',
                                text: response.message || 'Có lỗi xảy ra khi xóa danh mục game',
                            });
                        }
                    },
                    error: function(xhr) {
                        $('#deleteModal').modal('hide');
                        Swal.fire({
                            icon: 'error',
                            title: 'Không thể xóa',
                            text: xhr.responseJSON?.message || 'Có lỗi xảy ra khi xóa danh mục game',
                        });
                    }
                });
            });
        });
    </script>
@endpush
