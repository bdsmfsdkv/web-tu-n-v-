                                    ...
                                    @forelse($transactions as $transaction)
                                        <div class="order-card">
                                            <div class="order-card-title">
                                                <span class="iconify" data-icon="ant-design:file-text-outlined"></span> Thông tin đơn hàng #{{ $transaction->order_code }}
                                            </div>
                                            
                                            <div class="order-info-grid">
                                                <div class="order-info-box">
                                                    <div class="order-info-label">Danh mục</div>
                                                    <div class="order-info-val">{!! str_replace(' ', '<br>', $transaction->category->name ?? 'Tài khoản Game') !!}</div>
                                                </div>
                                                <div class="order-info-box">
                                                ...
                                                <div class="order-info-box">
                                                    ...
                                                    <div class="order-info-val" style="color: var(--primary); font-size: 1.05rem;">{{ number_format($transaction->price) }}đ</div>
                                                </div>
                                                <div class="order-info-box">
                                                ...
                                                <div class="order-info-box">
                                                    ...
                                                    <div class="order-info-val">{{ $transaction->created_at->format('d/m/Y') }}<br>{{ $transaction->created_at->format('H:i') }}</div>
                                                </div>
                                            </div>
                                            ...
                                            <div class="order-acc-list-title">
                                                ...
                                                <div>
                                                    <a href="{{ route('account.show', ['id' => $transaction->id]) }}" target="_blank" class="order-btn-copy" style="text-decoration: none;">
                                            ...
                                            <div style="display: flex; gap: 8px; margin-bottom: 12px; flex-wrap: wrap;">
                                                <button class="order-btn-copy-all" onclick="copyToClipboardText('{{ addslashes($transaction->account_name . "|" . $transaction->password) }}')">
                                                    <span class="iconify" data-icon="ant-design:copy-outlined"></span> Copy tất cả
                                                </button>
                                                <button class="order-btn-export" onclick="exportToTxt('{{ addslashes($transaction->account_name . "|" . $transaction->password) }}', 'don_hang_{{ $transaction->id }}.txt')">
                                            ...
                                            <div class="order-acc-item">
                                                ...
                                                <div class="order-acc-text">
                                                    {{ $transaction->account_name }} | {{ $transaction->password }}
                                                </div>
                                                <button class="order-btn-copy" onclick="copyToClipboardText('{{ addslashes($transaction->account_name . " | " . $transaction->password) }}')">
