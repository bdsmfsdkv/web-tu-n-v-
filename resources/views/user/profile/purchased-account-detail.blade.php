...
@php
    ...
    if (!$returnUrl && isset($transaction->category) && !empty($transaction->category->slug)) {
        $backCategoryUrl = route('category.index', ['slug' => $transaction->category->slug]);
                    ...
                    <div class="d-flex align-items-center gap-3">
                        ...
                        <div>
                            <h1 class="profile-title mb-1" style="font-size: 1.5rem; font-weight: 800;">
                                ĐƠN HÀNG #{{ $transaction->order_code ?? ('ACC-' . $transaction->id) }}
                            </h1>
                            <div class="d-flex align-items-center gap-2 text-muted small">
                                <span><i class="fa-regular fa-clock me-1"></i> {{ ($transaction->purchased_at ?? $transaction->created_at)->format('H:i - d/m/Y') }}</span>
                                <span>•</span>
                                <span>{{ ($transaction->purchased_at ?? $transaction->created_at)->diffForHumans() }}</span>
                            </div>
                        </div>
                    </div>
            ...
            <div class="profile-content">
                ...
                <div class="profile-main">
                    <style>
                        ...
                        [data-theme="dark"] .detail-item-box {
                            background: #18181b;
                            border-color: rgba(255, 255, 255, 0.06);
                        }
                        ...
                        <div class="order-meta-grid">
                            <div class="order-meta-box">
                                ...
                                <div class="order-meta-info">
                                    ...
                                    <div class="order-meta-val" title="{{ $transaction->category_name ?? ($transaction->category->name ?? 'Tài khoản Game') }}">
                            ...
                            <div class="order-meta-box">
                                ...
                                <div class="order-meta-info">
                                    ...
                                    <div class="order-meta-val">1 Tài khoản</div>
                                </div>
                            </div>
                            ...
                            <div class="order-meta-box">
                                ...
                                <div class="order-meta-info">
                                    ...
                                    <div class="order-meta-val">
                                        {{ ($transaction->purchased_at ?? $transaction->created_at)->format('H:i d/m/Y') }}
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <!-- Vault / Account Locker -->
                    <div class="vault-section">
                        @php
                            $fullAccStr = $transaction->account_name . ($transaction->password ? '|' . $transaction->password : '');
                        @endphp

                        <div class="vault-header">
                            <div class="vault-title">
                                <i class="fa-solid fa-key text-danger"></i>
                                Thông Tin Tài Khoản Đã Nhận
                            </div>
                            <div class="d-flex gap-2 flex-wrap">
                                <button type="button" class="btn-vault-action btn-vault-copy-all" onclick="copyToClipboardText('{{ addslashes($fullAccStr) }}', 'Đã sao chép TK|MK!')">
                                    <i class="fa-solid fa-copy"></i> Copy Cả Cặp
                                </button>
                                <button type="button" class="btn-vault-action btn-vault-export" onclick="exportToTxt('{{ addslashes($fullAccStr) }}', 'don_hang_{{ $transaction->id }}.txt')">
                        ...
                        <div class="acc-card">
                            <div class="acc-fields-grid">
                                ...
                                <div class="acc-field-box">
                                    <div class="acc-field-info">
                                        ...
                                        <div class="acc-field-val">{{ $transaction->account_name }}</div>
                                    </div>
                                    <button type="button" class="btn-field-action" title="Sao chép tài khoản" onclick="copyToClipboardText('{{ addslashes($transaction->account_name) }}', 'Đã sao chép tài khoản!')">
                                ...
                                <div class="acc-field-box">
                                    ...
                                    <div class="d-flex gap-1">
                                        <button type="button" class="btn-field-action" title="Hiện/Ẩn mật khẩu" onclick="togglePassDetail()">
                                            <i class="fa-solid fa-eye" id="detail-eye-icon"></i>
                                        </button>
                                        <button type="button" class="btn-field-action" title="Sao chép mật khẩu" onclick="copyToClipboardText('{{ addslashes($transaction->password) }}', 'Đã sao chép mật khẩu!')">
                                            <i class="fa-solid fa-clone"></i>
                                        </button>
                                    </div>
                                    @endif
                            ...
                            @if(!empty($transaction->details) && is_array($transaction->details))
                                <div class="mt-4 pt-3 border-top">
                                    <div class="fw-bold mb-2 small text-muted text-uppercase">
                                        <i class="fa-solid fa-circle-info text-primary me-1"></i> Thông tin bổ sung của tài khoản:
                                    </div>
                                    <div class="details-grid">
                                        @foreach($transaction->details as $detail)
                                            @if(is_array($detail) && isset($detail['key']))
                                                <div class="detail-item-box">
                                                    <span class="text-muted small fw-bold">{{ $detail['key'] }}:</span>
                            ...
                            @if($transaction->note)
                                <div class="mt-3 p-3 rounded" style="background: rgba(234, 179, 8, 0.08); border: 1px dashed rgba(234, 179, 8, 0.3); font-size: 0.88rem;">
                                    <strong class="text-warning"><i class="fa-solid fa-note-sticky me-1"></i> Ghi chú đơn hàng:</strong>
                                    <div class="mt-1">{!! nl2br(e($transaction->note)) !!}</div>
                                </div>
                            @endif
                        </div>

                        <div class="order-security-alert">
                            <i class="fa-solid fa-shield-halved"></i>
                            <div>
                                <strong>Lưu ý bảo mật:</strong> Hãy đăng nhập và đổi mật khẩu tài khoản ngay sau khi nhận để đảm bảo an toàn tuyệt đối cho tài khoản của bạn!
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
                        <a href="{{ route('profile.purchased-accounts') }}" class="btn btn-outline-secondary d-inline-flex align-items-center gap-2" style="border-radius: 10px; font-weight: 700;">
                            <i class="fa-solid fa-list-ul"></i> Tất Cả Tài Khoản Đã Mua
                        </a>
...
@push('scripts')
<script>
    function togglePassDetail() {
        var box = document.getElementById('detail-pass-box');
        var eye = document.getElementById('detail-eye-icon');
        if (!box) return;
