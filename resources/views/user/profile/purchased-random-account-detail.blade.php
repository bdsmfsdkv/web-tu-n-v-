        ...
        <div class="container">
            <div class="profile-container">
                <div class="profile-header">
                    <h1 class="profile-title"><i class="fa-solid fa-file-invoice me-2"></i> ĐƠN HÀNG #{{ str_replace('ORD-', '', str_replace('LEGACY-', '', $order->batch_id)) }}</h1>
                </div>
                    ...
                    <div class="profile-main">
                        <div class="profile-info-card">
                            <div class="info-header" style="margin-bottom: 20px;">
                                <a href="{{ route('profile.purchased-random-accounts') }}" style="color: #ef4444; text-decoration: none; font-weight: 600; display: flex; align-items: center; gap: 8px;">
                                    ...
                                    <div class="order-card">
                                        ...
                                        <div class="order-info-grid">
                                            <div class="order-info-box">
                                                ...
                                                <div class="order-info-val">{!! str_replace(' ', '<br>', $order->category->name ?? 'Tài khoản ngẫu nhiên') !!}</div>
                                            </div>
                                            <div class="order-info-box">
                                                <div class="order-info-label">Số lượng</div>
                                                <div class="order-info-val">{{ $order->quantity }} acc</div>
                                            </div>
                                            <div class="order-info-box">
                                                <div class="order-info-label">Tổng tiền</div>
                                                <div class="order-info-val" style="color: #ef4444; font-size: 1.05rem;">{{ number_format($order->total_price) }}đ</div>
                                            </div>
                                            <div class="order-info-box">
                                            ...
                                            <div class="order-info-box">
                                                ...
                                                <div class="order-info-val">{{ \Carbon\Carbon::parse($order->purchase_time)->format('d/m/Y') }}<br>{{ \Carbon\Carbon::parse($order->purchase_time)->format('H:i') }}</div>
                                            </div>
                                        </div>

                                        <div class="order-acc-list-title">
                                            <div class="order-acc-list-title-text">
                                                <span class="iconify" data-icon="ant-design:key-outlined"></span> Danh sách tài khoản ({{ $order->quantity }})
                                            </div>
                                        </div>
                                        ...
                                        <div style="display: flex; gap: 8px; margin-bottom: 16px; flex-wrap: wrap;">
                                            ...
                                            @endphp
                                            <button class="order-btn-copy-all" onclick="copyToClipboardText('{{ addslashes(str_replace("\n", "\\n", $allAccountsStr)) }}')">
                                                <span class="iconify" data-icon="ant-design:copy-outlined"></span> Copy tất cả
                                            </button>
                                            <button class="order-btn-export" onclick="exportToTxt('{{ addslashes(str_replace("\n", "\\n", $allAccountsStr)) }}', 'don_hang_random_{{ str_replace('ORD-', '', $order->batch_id) }}.txt')">
                                        ...
                                        @foreach($order->accounts as $index => $acc)
                                            <div class="order-acc-item">
                                                <div class="order-acc-index">{{ $index + 1 }}</div>
                                                <div class="order-acc-text">
                                                    {{ $acc->account_name }}@if($acc->password) | {{ $acc->password }}@endif
                                                </div>
                                                <button class="order-btn-copy" onclick="copyToClipboardText('{{ addslashes($acc->account_name . ($acc->password ? '|' . $acc->password : '')) }}')">
