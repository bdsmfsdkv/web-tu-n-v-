...
class DiscountCodeController extends Controller
{
    ...
    public function validateCode(Request $request)
    {
        ...
        $unitPrice = $this->getOriginalPrice($context, $itemId);
        
        // For random accounts purchased from category, apply flash sale price if active
        if ($context === 'random_account') {
            $flashSalePrice = \App\Models\FlashSale::getActivePrice('random', $itemId);
            if ($flashSalePrice !== null) {
                $unitPrice = $flashSalePrice;
            }
        }
        
        // Same for game accounts
        if ($context === 'account') {
    ...
    private function getOriginalPrice($context, $itemId)
    {
        switch ($context) {
            ...
            case 'random_account':
                // For random_account, itemId can be either the category ID (when buying from category) 
                // or the account ID (if buying specific). Let's assume it's category ID for now based on the UI.
                $randomAccount = DB::table('random_category_accounts')
                    ->where('random_category_id', $itemId)
