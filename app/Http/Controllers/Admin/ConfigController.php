...
class ConfigController extends Controller
{
    ...
    /**
     * Hiển thị trang cấu hình hệ thống tổng hợp
     */
    public function index(\Illuminate\Http\Request $request)
    {
        ...
        $configs = [
            ...
            'top_deposit_reward' => config_get('top_deposit_reward', '<p>Phần thưởng nạp thẻ đang được cập nhật...</p>'),
            'min_withdraw_gold' => config_get('min_withdraw_gold', 1000),
            'max_withdraw_gold' => config_get('max_withdraw_gold', 1000000000),
            ...
            // terms
            'terms_of_use' => config_get('terms_of_use', ''),
            'privacy_policy' => config_get('privacy_policy', ''),
        ];

        return view('admin.settings.index', compact('title', 'configs'));
    }
    ...
    public function updateGeneral(Request $request)
    {
        $request->validate([
            ...
            'top_deposit_reward' => 'nullable|string',
        ...
        try {
            ...
            if ($request->has('remove_banners') && is_array($request->remove_banners)) {
                ...
                if (!is_array($currentBanners)) {
                    if (config_get('site_banner')) {
                        $currentBanners = [config_get('site_banner')];
                    } else {
                        $currentBanners = [];
                    }
                }
            ...
            DB::commit();
            return redirect()->route('admin.settings.index')->with('tab', 'general')
    ...
    public function updateSocial(Request $request)
    {
        ...
        try {
            ...
            DB::commit();
            return redirect()->route('admin.settings.index')->with('tab', 'social')
    ...
    public function updateEmail(Request $request)
    {
        ...
        try {
            ...
            DB::commit();
            return redirect()->route('admin.settings.index')->with('tab', 'email')
    ...
    public function updatePayment(Request $request)
    {
        ...
        try {
            ...
            DB::commit();
            return redirect()->route('admin.settings.index')->with('tab', 'payment')
    ...
    public function updateLogin(Request $request)
    {
        ...
        try {
            ...
            return redirect()->route('admin.settings.index')->with('tab', 'login')
    ...
    /**
     * Hiển thị trang cấu hình điều khoản và chính sách
     */
    public function terms()
    {
        ...
        return view('admin.settings.terms', compact('title', 'configs'));
    }

    /**
     * Cập nhật cấu hình điều khoản và chính sách
     */
    public function updateTerms(Request $request)
    {
        ...
        try {
            ...
            config_set('terms_of_use', $request->terms_of_use);
            config_set('privacy_policy', $request->privacy_policy);

            config_clear_cache();

            DB::commit();

            return redirect()->route('admin.settings.index')->with('tab', 'terms')
                ->with('success', 'Điều khoản và chính sách đã được cập nhật thành công.');
        } catch (\Exception $e) {
            DB::rollBack();
            Log::error('Lỗi cập nhật điều khoản: ' . $e->getMessage());
            return redirect()->back()
                ->with('error', 'Đã xảy ra lỗi khi cập nhật: ' . $e->getMessage())
                ->withInput();
        }
    }
}
