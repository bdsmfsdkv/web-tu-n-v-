    ...
    <div class="row align-items-center">
        <div class="col-md-12">
            <div class="page-header-title">
                ...
                <p class="text-muted">Quản lý mã giảm giá cho tài khoản và dịch vụ</p>
                ...
                <div class="page-btn">
                    <a href="<?php echo e(route('admin.discount-codes.create')); ?>" class="btn btn-success">
                        ...
                        <table class="table table-hover table-borderless align-middle mb-0 text-nowrap w-100">
                            <thead class="bg-light-subtle text-muted">
                                <tr>
                                    ...
                                    <th class="text-uppercase small">Kiểu</th>
                                    <th class="text-uppercase small">Giá trị</th>
                                    <th class="text-uppercase small">Lượt sử dụng còn lại</th>
                                    <th class="text-uppercase small">Hết hạn</th>
                                    <th class="text-uppercase small">Trạng thái</th>
                                    <th class="text-end">Thao tác</th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php $__empty_1 = true; $__currentLoopData = $discountCodes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $discountCode): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                    <tr>
                                        <td>
                                            <div class="form-check"><input class="form-check-input" type="checkbox"></div>
                                        </td>
                                        <td><?php echo e($key + 1); ?></td>
                                        <td class="text-bolds"><?php echo e($discountCode->code); ?></td>
                                        <td><?php echo e($discountCode->discount_type === 'percentage' ? 'Phần trăm' : 'Số tiền cố định'); ?>

                                        </td>
                                        <td>
                                            <?php if($discountCode->discount_type === 'percentage'): ?>
                                                <?php echo e($discountCode->discount_value); ?>%
                                            <?php else: ?>
                                                <?php echo e(number_format($discountCode->discount_value)); ?>đ
                                            <?php endif; ?>
                                        </td>
                                        <td><?php echo e($discountCode->usage_limit); ?></td>
                                        <td><?php echo e($discountCode->expire_date ? date('d/m/Y', strtotime($discountCode->expire_date)) : 'Không hết hạn'); ?>

                                        </td>
                                        <td>
                                            <span
                                                class="badge <?php echo e($discountCode->is_active === '1' ? 'bg-lightgreen' : 'bg-lightred'); ?>">
                                                <?php echo e($discountCode->is_active === '1' ? 'Hoạt động' : 'Không hoạt động'); ?>

                                            </span>
                                        </td>
                                        <td class="text-end">
                                            <a class="me-3"
                                                href="<?php echo e(route('admin.discount-codes.edit', $discountCode->id)); ?>"
                                                title="Chỉnh sửa">
                                                <i class="ti ti-edit fs-5 text-primary"></i>
                                            </a>
                                            <a class="me-3 delete-item" href="javascript:void(0);"
                                                onclick="showDeleteModal(<?php echo e($discountCode->id); ?>)" title="Xóa">
...
<?php $component = App\View\Components\ModalConfirmDelete::resolve(['message' => 'Bạn có chắc chắn muốn xóa mã giảm giá này không? Tất cả dữ liệu có liên quan đến mã giảm giá này sẽ
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#confirmDelete').on('click', function() {
                $.ajax({
                    url: "<?php echo e(route('admin.discount-codes.destroy', ':id')); ?>".replace(':id',
                        discountCodeId),
                    type: 'DELETE',
                    data: {
...
<?php echo $__env->make('layouts.admin.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\SHOP BÁN ACC GAME V1\resources\views/admin/discount-codes/index.blade.php ENDPATH**/ ?>