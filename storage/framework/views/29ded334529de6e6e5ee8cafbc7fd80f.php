...
<?php $component = App\View\Components\HeroHeader::resolve(['title' => ''.e(strtoupper($category->name)).'','description' => ''] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? (array) $attributes->getIterator() : [])); ?>
        ...
        <div class="container">
            ...
            <div class="card" style="margin-bottom: 24px; background: var(--bg-card); border: 1px solid var(--border-color); border-radius: 12px; padding: 20px;">
                <div style="color: var(--text-color); line-height: 1.6;">
                    <?php echo nl2br(e($category->description)); ?>
            ...
            <div class="card" style="background: var(--bg-card); border: 1px solid var(--border-color); border-radius: 12px; padding: 24px;">
                <div style="display: flex; gap: 24px; flex-wrap: wrap;">
                    <div style="flex: 0 0 auto; width: 260px; max-width: 100%;">
                        <img src="<?php echo e(!empty($category->thumbnail) ? asset($category->thumbnail) : 'https://via.placeholder.com/300x180'); ?>" alt="<?php echo e($category->name); ?>" style="width: 100%; border-radius: 12px; object-fit: cover; border: 1px solid var(--border-color);">
                    </div>
                    <div style="flex: 1; min-width: 280px; display: flex; flex-direction: column; justify-content: center;">
                        ...
                        <p style="color: var(--text-muted); margin: 0 0 16px 0; font-size: 0.95rem;">Bạn sẽ nhận được 1 tài khoản ngẫu nhiên từ danh mục <strong><?php echo e($category->name); ?></strong></p>
                        ...
                        <div style="display: flex; gap: 24px; margin-bottom: 24px; align-items: center; font-size: 0.95rem;">
                            <div>
                                ...
                                <span style="color: #10b981; font-weight: 600;"><?php echo e(number_format($availableCount)); ?> acc</span>
                            </div>
                            <div>
                                <span style="color: var(--text-muted);">Giá: </span>
                                <span style="color: #ef4444; font-weight: 700; font-size: 1.1rem;"><?php echo e(number_format($price)); ?>đ</span>
                            </div>
                        </div>
        ...
        <div style="background: var(--bg-card); border: 1px solid var(--border-color); border-radius: 16px; width: 100%; max-width: 480px; animation: modalPop 0.2s ease-out; display: flex; flex-direction: column;">
            ...
            <div style="padding: 24px;">
                <div style="display: flex; gap: 16px; background: rgba(0,0,0,0.2); padding: 16px; border-radius: 12px; margin-bottom: 24px; border: 1px solid var(--border-color);">
                    <img src="<?php echo e(!empty($category->thumbnail) ? asset($category->thumbnail) : 'https://via.placeholder.com/150'); ?>" style="width: 60px; height: 60px; border-radius: 8px; object-fit: cover;">
                    <div style="display: flex; flex-direction: column; justify-content: center;">
                        <h4 style="margin:0 0 4px 0; font-size: 1.05rem; font-weight: 600; color: var(--text-color);"><?php echo e($category->name); ?></h4>
                        <div style="color: var(--text-muted); font-size: 0.9rem;">
                            Giá: <span style="color:#ef4444; font-weight:600;"><?php echo e(number_format($price)); ?>đ</span>/acc
                        </div>
                        <div style="color: var(--text-muted); font-size: 0.85rem; margin-top: 2px;">
                            Còn <?php echo e(number_format($availableCount)); ?> tài khoản
                        </div>
                    </div>
                </div>
                ...
                <div style="margin-bottom: 24px;">
                    ...
                    <div style="display: flex; gap: 10px;">
                        ...
                        <input type="number" id="purchaseQuantity" value="1" min="1" max="<?php echo e($availableCount); ?>" onchange="updateTotalPrice()" style="width: 80px; height: 44px; text-align: center; border-radius: 8px; border: 1px solid var(--border-color); background: rgba(0,0,0,0.2); color: var(--text-color); font-weight: 600; font-size: 1.1rem; outline: none;">
                        <button onclick="changeQuantity(1)" style="width: 44px; height: 44px; border-radius: 8px; border: 1px solid var(--border-color); background: rgba(0,0,0,0.2); color: var(--text-color); font-size: 1.2rem; cursor: pointer; display: flex; align-items: center; justify-content: center;">+</button>
                    </div>
                </div>
                ...
                <div style="display: flex; justify-content: space-between; align-items: center; padding: 16px 0; border-top: 1px solid var(--border-color); border-bottom: 1px solid var(--border-color); margin-bottom: 24px;">
                    ...
                    <span id="totalPriceDisplay" style="color: #ef4444; font-weight: 700; font-size: 1.3rem;"><?php echo e(number_format($price)); ?>đ</span>
                </div>
...
<script>
    const unitPrice = <?php echo e($price); ?>;
    const maxQty = <?php echo e($availableCount); ?>;
    
    function openPurchaseModal() {
        <?php if(auth()->guard()->check()): ?>
            document.getElementById('randomPurchaseModal').style.display = 'flex';
            // Initialize discount handler
            initDiscountHandler('random_account', <?php echo e($category->id); ?>, unitPrice);
        <?php else: ?>
    ...
    function submitPurchase() {
        ...
        fetch("<?php echo e(route('random.category.purchase', $category->slug)); ?>", {
            method: 'POST',
            headers: {
...
<?php echo $__env->make('layouts.user.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\resources\views\user\random\category.blade.php ENDPATH**/ ?>