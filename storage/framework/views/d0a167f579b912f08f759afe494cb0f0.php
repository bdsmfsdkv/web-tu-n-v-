    ...
    <div class="row align-items-center">
        <div class="col-md-12">
            <div class="page-header-title">
                <h2 class="mb-0">Quản lý vòng quay may mắn</h2>
                <p class="text-muted">Xem và quản lý tất cả vòng quay may mắn</p>
                ...
                <div class="page-btn">
                    <a href="<?php echo e(route('admin.lucky-wheels.create')); ?>" class="btn btn-success">
            ...
            <div class="card overflow-hidden shadow-sm border border-dashed">
                <div class="card-header border-bottom-0 pt-4 pb-0 d-flex justify-content-between align-items-center">
                    <h5 class="card-title mb-0 text-primary">
                        <i class="ti ti-aperture text-primary me-2"></i>Danh sách vòng quay
                    </h5>
                    <a href="<?php echo e(route('admin.lucky-wheels.history')); ?>" class="btn btn-sm btn-light-info text-info">
                        <i class="ti ti-history me-1"></i> Xem lịch sử quay
                    </a>
                </div>
                        ...
                        <table class="table table-hover table-borderless align-middle mb-0 text-nowrap w-100">
                            <thead class="bg-light-subtle text-muted">
                                <tr>
                                    ...
                                    <th class="text-uppercase small">Tên vòng quay</th>
                                    <th class="text-uppercase small text-center">Hình ảnh</th>
                                    <th class="text-uppercase small text-end">Giá/Lượt quay</th>
                            ...
                            <tbody>
                                <?php $__empty_1 = true; $__currentLoopData = $luckyWheels; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $wheel): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                    <tr>
                                        <td><span class="fw-medium">#<?php echo e($wheel->id); ?></span></td>
                                        <td><span class="fw-bold text-auto"><?php echo e($wheel->name); ?></span></td>
                                        <td class="text-center">
                                            <img src="<?php echo e($wheel->thumbnail); ?>" alt="<?php echo e($wheel->name); ?>" style="width: 80px; height: 80px; object-fit: cover;" class="rounded border shadow-sm">
                                        </td>
                                        <td class="text-end fw-bold text-primary"><?php echo e(number_format($wheel->price_per_spin)); ?> đ</td>
                                        <td>
                                            <?php if($wheel->active): ?>
                                                <span class="badge bg-light-success text-success fw-bold px-3 py-2 rounded-pill">Hoạt động</span>
                                            <?php else: ?>
                                                <span class="badge bg-light-danger text-danger fw-bold px-3 py-2 rounded-pill">Đã tắt</span>
                                            <?php endif; ?>
                                        </td>
                                        <td><span class="text-muted small"><?php echo e($wheel->created_at->format('d/m/Y H:i')); ?></span></td>
                                        <td class="text-end">
                                            <div class="btn-group">
                                                <a class="btn btn-sm btn-light-primary text-primary" href="<?php echo e(route('admin.lucky-wheels.edit', $wheel->id)); ?>"
                                                    title="Chỉnh sửa">
                                                    <i class="ti ti-edit"></i>
                                                </a>
                                                <a class="btn btn-sm btn-light-danger text-danger delete-item" href="javascript:void(0);"
                                                    data-id="<?php echo e($wheel->id); ?>" title="Xóa">
...
<?php $component = App\View\Components\ModalConfirmDelete::resolve(['message' => 'Bạn có chắc chắn muốn xóa vòng quay may mắn này không? Tất cả dữ liệu có liên quan đến nó sẽ
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            $('#confirmDelete').on('click', function() {
                $.ajax({
                    url: "<?php echo e(route('admin.lucky-wheels.destroy', '')); ?>/" + luckyWheelId,
                    type: 'DELETE',
                    data: {
...
<?php echo $__env->make('layouts.admin.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\SHOP BÁN ACC GAME V1\resources\views/admin/lucky-wheels/index.blade.php ENDPATH**/ ?>